<template>
      ...
      Object.assign(
        {
          ...
          'x-drag-dom':
            from === 'th' && rowIndex === 0 && dragColumns.includes(columnItem.dataIndex)
        },
        getCustomCell('class')
      )
    "
  ...
  >
    <div
      ...
      :style="{
        width: isSpanExpand ? null : getTdWidth(columnIndex, getColSpan)
      }"
    >
      <slot
      ...
      <span v-if="from === 'th' && rowIndex === 0 && columnItem.sort" class="x-sort">
        <slot
      ...
      <span v-if="from === 'th' && rowIndex === 0 && columnItem.search" class="x-search">
        <slot
      ...
      <span v-if="from === 'th' && rowIndex === 0 && columnItem.filter" class="x-filter">
        <slot
  ...
  computed: {
    ...
    getTdStyle() {
      ...
      const style = this.getStickyDirection(this.columnItem.dataIndex)
        ? {
            [this.getStickyDirection(this.columnItem.dataIndex)]: this.getStickyDistance(
              this.columnIndex,
              this.columnLength - 1,
              this.getStickyDirection(this.columnItem.dataIndex)
            ),
