<template>
  <div class="x-table-wrapper" :id="domInit.wrap">
    ...
    <div class="x-table-area" v-else>
      <div v-if="!config.isUseSingleTable" class="x-table-fix-header" :style="stickyStyle">
        <div
          :id="domInit.header"
        ...
        >
            ...
            <thead class="x-head-dom">
              ...
              >
                <template
                  ...
                  :key="`th-${config.key}-${titleIndex}-${columnItem.dataIndex}`"
                >
                  <x-td
                    from="th"
                    :is-sticky="config.isSticky"
                  ...
                  >
                    <template v-slot:[`th-${columnItem.dataIndex}`]="{ record, text }">
                      <span v-if="columnItem.dataIndex === 'checkbox'">
                        <a-checkbox v-model:checked='isCheckboxSelectAll' @change="handleTHCheckbox"/>
                      </span>
                      <slot
          ...
          <div
            v-if="config.isSticky && stickyLeftColumns.length > 0"
            :id="domInit.leftHeader"
            class="x-sticky-shadow x-sticky-left-shadow"
            :style="{
              left: getStickyDistance(
          ...
          ></div>
          <div
            v-if="config.isSticky && stickyRightColumns.length > 0"
            :id="domInit.rightHeader"
            class="x-sticky-shadow x-sticky-right-shadow"
            :style="{
              left: getStickyDistance(
                tableColumns.length - 1 - stickyRightColumns.length,
                tableColumns.length - 1,
                'right',
                'shadow'
              )
      ...
      <div :id="domInit.content" class="x-table-content" :class="contentBorder" :style="styleTable">
        <div>
          <table class="x-fixed-table">
            <colgroup>
              <col
            ...
            <thead v-if="config.isUseSingleTable" class="x-head-dom">
              <tr
                v-for="(titleItem, titleIndex) in title"
                :key="`thr-${config.key}-${titleIndex}`"
                :class="titleItem.$class"
              ...
              <template v-if="showData.length <= 0">
                <tr>
                  <td class="x-no-data" :colspan="columns.length">
                    <slot name="noresult"> no result </slot>
                  </td>
                </tr>
              </template>
              <template v-else>
                <tr
                  ...
                  :key="`tdr-${config.key}-${rowItem[config.rowKey]}`"
                ...
                >
                  <template v-if="rowItem.$expandType && rowItem.$expandType === 'span'">
                    <x-td
                      :key="`td-${config.key}-${rowItem[config.rowKey]}-${getExpandSpanColumnData.dataIndex}`"
                      from="td"
                      :is-span-expand="true"
                      :is-sticky="config.isSticky"
                      :padding-length="paddingLength"
                      ...
                      :row-index="rowIndex"
                      :row-item="rowItem"
                      :clean-row-item="getCleanRowItem('tdSpan', rowItem)"
                      :column-index="0"
                  ...
                  <template v-else>
                    <template
                      ...
                      :key="`td-${config.key}-${rowItem[config.rowKey]}-${columnItem.dataIndex}`"
                    >
                      <x-td
                        from="td"
                        :is-sticky="config.isSticky"
                        ...
                        :row-index="rowIndex"
                        :row-item="rowItem"
                        :clean-row-item="getCleanRowItem(rowItem.$expandType ? 'tdExpand' : 'td', rowItem)"
                        :column-index="columnIndex"
                        :column-item="columnItem"
                        :colgroup-data="colgroupData"
                        :sticky-left-columns="stickyLeftColumns"
                        ...
                        <template v-slot:[`td-${columnItem.dataIndex}`]="{ record, text, expand }">
                          <span v-if="columnItem.dataIndex === 'checkbox'">
                            <a-checkbox
                              ...
                              @change="(selected) => handleTDCheckbox(selected, record)"
                            />
                          </span>
                          <slot
    ...
    <div class="x-page-area" v-if="showData.length > 0">
      <slot name="page" :total="dataAmount"></slot>
    </div>
  </div>
</template>

<script>
import { cloneDeep, throttle, isEmpty, forEach } from 'lodash';
...
export default {
  ...
  props: {
    ...
    // isSticky - 是否启用列固定，打开后columns里设置的sticky才有效果
    // isUseSingleTable - 是否使用单表格，高度固定时可以启用，避免渲染多个 table
    // expendFilterSearchResultShowType - 查询结果显示 open 全部打开 fit 符合的打开 close 全部关闭
    config: Object, // 表格的配置
    pageData: Object, // 分页数据 { page: 当前页码，windows：每页的条数 }
  ...
  computed: {
    resizeMin() {
      return this.config.resizeMin > 80 ? this.config.resizeMin : 80;
    },
    resizeMax() {
      const max = parseInt(this.wrapAreaWidth, 10) - this.resizeMin;
      return this.config.resizeMax < max ? this.config.resizeMax : max;
    },
    /**
    ...
    originExpandData() {
      const backData = {};
      if (this.expandData && !isEmpty(this.expandData)) {
        forEach(this.expandData, (useExpandData) => {
          forEach(useExpandData.data, (item) => {
            backData[item[this.config.rowKey]] = item;
          })
        })
      }
    ...
    contentBorder() {
      ...
      switch (String(this.config.border)) {
        case '0':
          break;
        case '1':
          if (this.config.isUseSingleTable) {
  ...
  methods: {
    ...
    initDom() {
      ...
      if (this.domMap.content) {
        // 如果开启表头固定
        if (!this.config.isUseSingleTable && this.isFixHeader) {
          if (this.domMap?.content?.getBoundingClientRect) {
            this.showFixHeader(this.domMap.content.getBoundingClientRect().top);
          } else {
            this.showFixHeader(0);
          }
          this.getScrollParent().addEventListener('scroll', () => {
            if (this.domMap?.content?.getBoundingClientRect) {
              this.showFixHeader(
                this.domMap.content.getBoundingClientRect().top,
                this.domMap.content.getBoundingClientRect().bottom
              );
            } else {
              this.showFixHeader(0, 0);
            }
          });
        }
        // 初始 stick 左右阴影是否显示
    ...
    autoResizeLastColumn() {
      ...
      const restWidth =
        ...
        this.colgroupData
          ...
          .reduce((a, b) => a + b, 0) -
        6;
      const minWidth = parseFloat(this.resizeMin);
      if (this.colgroupData[lastIndex]) {
        this.colgroupData[lastIndex].width =
          restWidth > minWidth ? `${restWidth}px` : `${minWidth}px`;
      }
    },
    /**
     * 表头切换的时候重新绑定事件
    ...
    initResizeAction() {
      ...
      this.$nextTick(() => {
        if (this.resizeColumns && this.resizeColumns.length > 0) {
          ...
          const resizeDom = this.$el.querySelectorAll('.x-resize-dom-handle');
    ...
    initColumnWidth() {
      ...
      const availableWidth = this.wrapAreaWidth - 2 * totalLength;
      ...
      this.columns.forEach((column, index) => {
        ...
        if (column.width) {
          ...
          curColumnWidth = parseFloat(minWidth);
        }
      ...
      if (totalColumnWidth < availableWidth) {
        const lastWithoutSettleWidth = availableWidth - settleColumnWidth;
        ...
        if (emptyColumnLength > 0) {
          ...
          this.tableColumns[settleWidthColumns[settleColumnLength - 1]].width = `${
            availableWidth - withOutLastColumnWidth
          }px`;
        }
      } else {
        emptyWidthColumns.forEach(itemIndex => {
          this.tableColumns[itemIndex].width = this.config.minWidth;
        });
        settleWidthColumns.forEach(itemIndex => {
          this.tableColumns[itemIndex].width = this.columns[itemIndex].width;
        });
      }
    },
    /**
    ...
    showLineShadow() {
      ...
      const rightDistance =
        this.domMap.content.clientWidth + this.domMap.content.scrollLeft ===
        this.domMap.content.scrollWidth;
    ...
    */
    ...
    getExpandStatus(rowData) {
      ...
      let backData = '';
      if (this.expandData && this.expandData[expandKey]) {
    ...
    getResize(e) {
      ...
      // 其他左侧 sticky 列的宽度
      let otherStickyLeftColumnsWidth = this.colgroupData
        .filter(
          (d, i) =>
            this.stickyLeftColumns.indexOf(this.columns[i].dataIndex) !== -1 &&
            this.initResize.index !== i
        )
        .reduce((c, d) => c + parseFloat(d.width), 0);
      if (otherStickyLeftColumnsWidth + width > this.resizeMax) {
        width = this.resizeMax - otherStickyLeftColumnsWidth;
      } else if (width > this.resizeMax) {
        width = this.resizeMax;
      } else if (width < this.resizeMin) {
        width = this.resizeMin;
      }
     ...
     */
    doDrag() {
      const wrapperTr = this.$el.querySelector('.x-head-dom tr');
      if (wrapperTr) {
        ...
        Sortable.create(wrapperTr, {
          ...
          onMove(evt) {
            ...
            if (!(dragGroupTo && dragGroupFrom && dragGroupTo === dragGroupFrom)) {
              ...
              const toLeft = evt.related.style.left;
              const fromLeft = evt.dragged.style.left;
              if (toLeft || fromLeft) {
                const children = [...wrapperTr.children].filter(
                  e => e.attributes?.group?.value === dragGroupTo
                );
                const lefts = children.map(e => e.style.left);
                const toIndex = children.findIndex(e => e === evt.related);
                const fromIndex = children.findIndex(e => e === evt.dragged);
                const [oldEl] = children.splice(fromIndex, 1);
                children.splice(toIndex, 0, oldEl);
          ...
          onEnd({ newIndex, oldIndex }) {
            if (newIndex !== oldIndex) {
              ...
              that.tableColumns.splice(oldIndex, 1);
              that.tableColumns.splice(newIndex, 0, oldItem);
              that.tableColumnsHead.splice(oldIndex, 1);
              that.tableColumnsHead.splice(newIndex, 0, oldItem);
              that.colgroupData.splice(oldIndex, 1);
              that.colgroupData.splice(newIndex, 0, oldGroupItem);
              that.setColumnsStorage();
              that.initResizeAction();
              // that.autoResizeLastColumn();

              that.emitTo('columnChange', {
                column: oldItem,
                oldIndex,
                newIndex
              });
    ...
    getSearchFilterData(from, data, filterSearchData) {
      ...
      return data.filter(itemData => {
        ...
        if (
          backData &&
          this.config.expendFilterSearchResultShowType === 'open' &&
          this.expandData[rowKey]
        ) {
    ...
    getSearchFilterExpandData(from, filterSearchData) {
      ...
      if (
        ...
        this.expandJoinFilterSearchColumns.length > 0 &&
        expandData
      ) {
        ...
        if (Object.keys(usedFilterSearchData).length > 0) {
          Object.keys(expandData).forEach(rowKey => {
            ...
            if (
              curExpandData &&
              curExpandData.type === 'data' &&
              curExpandData.data &&
              curExpandData.data.length > 0
            ) {
            ...
            if (curExpandHasResult) {
              if (this.config.expendFilterSearchResultShowType !== 'close') {
        ...
        for (let i = dataLength; i >= 1; i -= 1) {
          ...
          if (isSameGroup && value === preValue) {
            ...
            backData[i][dataIndex] = {
              rowSpan: 0,
              colSpan:
                typeof data[i][dataIndex].colSpan !== 'undefined' ? data[i][dataIndex].colSpan : 1,
              value,
              groupIndex
            };
          } else {
          ...
          if (i === 1) {
            ...
            backData[i - 1][dataIndex] = {
              rowSpan: spanRowNumber,
              colSpan:
                typeof data[i - 1][dataIndex].colSpan !== 'undefined'
                  ? data[i - 1][dataIndex].colSpan
                  : 1,
              value: preValue,
              groupIndex
            };
    ...
    handleTHCheckbox(selected) {
      let curSelectedData= [...this.checkboxSelectedData];
      ...
      if (isSelected) {
        ...
        forEach(this.showData, (itemData) => {
          const itemKey = itemData[this.config.rowKey];
          if (curSelectedData.includes(itemKey)) {
            const index = curSelectedData.findIndex((selected) => {
...
.x-table-wrapper {
  .x-table-area {
    ...
    .x-sticky-shadow {
      ...
      z-index: 998;
      top: 48px;
      background-color: rgba(173, 179, 190, 0.05);
    }
